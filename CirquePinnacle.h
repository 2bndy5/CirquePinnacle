...
/**
 * @defgroup DataMode the Data Mode
 * @brief Allowed symbols for configuring the Pinanacle ASIC's data
...
/**
 * @defgroup AnyMeasGain AnyMeas Gain
 * @brief Allowed ADC gain configurations of AnyMeas mode.
 * @remark The percentages defined here are approximate values.
...
/**
 * @defgroup AnyMeasFreq AnyMeas Frequncy
 * @brief Allowed frequency configurations of AnyMeas mode.
 * @remark The frequencies defined here are approximated based on an aperture
...
/**
 * @defgroup AnyMeasMuxing AnyMeas Matrix Muxing
 * @brief Allowed muxing gate polarity and reference capacitor configurations
 * of AnyMeas mode.
 * @remark Combining these values (with `+` operator) is allowed.
...
/**
 * @defgroup AnyMeasCtrl AnyMeas Control
 * @brief These constants control the number of measurements performed in
 * PinnacleTouch::measureADC().
 * @remark The number of measurements can range [0, 63].
 * @sa PinnacleTouch::anyMeasModeConfig()
 */
///@{
/**
...
public:
     ...
     * not. It only applies to @ref PINNACLE_RELATIVE or @ref PINNACLE_ABSOLUTE
     * mode, otherwise if @ref DataMode is set to @ref PINNACLE_ANYMEAS, then this
     ...
     * reported or not.
     * @returns The setting configured by feedEnabled() or `false` if @ref DataMode
     ...
     * or @ref PINNACLE_ANYMEAS (referred to as "AnyMeas" in specification sheet)
     * mode for reading ADC values. Invalid input values have no affect.
     */
     ...
     * reportRelative() or reportAbsolute() depending on what @ref DataMode is set
     * to.
     ...
     * if @ref DataMode is set to PINNACLE_ANYMEAS, then this function does
     * nothing.
     * @param[out] report A reference pointer (declared variable of datatype
     ...
     * taken. This function only applies to @ref PINNACLE_RELATIVE or
     * @ref PINNACLE_ABSOLUTE mode, otherwise if @ref DataMode is set to
     * @ref PINNACLE_ANYMEAS, then this function will do nothing.
     ...
     * This function only applies to @ref PINNACLE_RELATIVE or
     * @ref PINNACLE_ABSOLUTE mode, otherwise if @ref DataMode is set to
     * @ref PINNACLE_ANYMEAS, then this function will do nothing.
     ...
     * for compensation calculations when measuring of input events. See note
     * below from the Pinnacle ASIC's application note in
     * getCalibrationMatrix() about deciding what values to use.
     */
     ...
     * @par A note from Cirque's Application Note on Comparing matrices:
    ...
    */
     ...
     * sure to set the @ref DataMode attribute to @ref PINNACLE_ANYMEAS before
     ...
     * parameters passed to startMeasureAdc(). Be sure that the @ref DataMode
     * attribute is set to @ref PINNACLE_ANYMEAS and available() returns `true`
     * before calling this function otherwise it will return `0`.
     * @returns
     * * A 16-bit integer if available() returns `true` and if @ref DataMode is
     *   set to @ref PINNACLE_ANYMEAS.
     * * `0` if @ref DataMode is not set to @ref PINNACLE_ANYMEAS or if available()
     *   returns `false`.
     */
...
protected:
     ...
     *       operations will be nullified by setting @ref DataMode to `0xFF`).
     */
    bool begin();
};
